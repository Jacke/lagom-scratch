...
class MicroserviceCalEntitySpec extends WordSpec with Matchers with BeforeAndAfterAll {

  private val system = ActorSystem("MicroserviceCalEntitySpec",
    JsonSerializerRegistry.actorSystemSetupFor(MicroserviceCalSerializerRegistry))

  override protected def afterAll(): Unit = {
    TestKit.shutdownActorSystem(system)
  }

  private def withTestDriver(block: PersistentEntityTestDriver[MicroserviceCalCommand[_], MicroserviceCalEvent, MicroserviceCalState] => Unit): Unit = {
    val driver = new PersistentEntityTestDriver(system, new MicroserviceCalEntity, "microservice-cal-1")
    block(driver)
